...
class Cortex(Dispatcher):
    ...
    def sub_request(self, stream):
        ...
        while True:
            ...
            result_dic = json.loads(new_data)
            print(json.dumps(result_dic, indent=4))
            if result_dic['fac'][0] in ['winkL','blink', 'winkR']: # and result_dic['fac'][4] > 0.8:
                count += 1
                if count > NUM:
                    count = 0
                    pos += 1

            # elif result_dic['fac'][0] == 'winkR': # and result_dic['fac'][4] > 0.8:
            #     count -= 1
            #     if count < -NUM:
            #         count = 0
            #         pos += 1
